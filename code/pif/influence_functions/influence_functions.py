...
def calc_s_test(
    ...
    # print(start, min(end, len(test_loader)))

    #     for i in range(start, min(end, len(test_loader))):
    for i, data in enumerate(test_loader):
        inputs, values, labels = data
        #         print(save.joinpath(f"did-{int(did)}_tid-{int(tid)}_recdep{recursion_depth}_r{r}.s_test").exists())
        #         if save.joinpath(f"did-{int(did)}_tid-{int(tid)}_recdep{recursion_depth}_r{r}.s_test").exists():
        #             continue
        # breakpoint()
        #         z_test = test_loader.collate_fn([inputs])
        #         m_test = torch.zeros_like(inputs)
        #         m_test = test_loader.collate_fn([m_test])
        #         t_test = test_loader.collate_fn([labels])

        z_test = Variable(inputs.type(torch.FloatTensor)).cuda()
        m_test = torch.zeros_like(inputs)
        m_test = torch.tensor(m_test, dtype=torch.float32).cuda()
        t_test = torch.tensor(labels, dtype=torch.long).cuda()
...
def calc_grad_z(model_name, model, train_loader, save_pth=False, gpu=-1, start=0, end=0):
...
#     print(model)
    ptuples = tuple(model.named_parameters())
    grad_zs = []
    #     for i in range(start, min(end, len(train_loader))):
    for i, data in enumerate(train_loader):
        # print('calc_grad_z', i, len(train_loader))
        
        if model_name not in ['roberta', 'longformer']:
            inputs, values, labels = data
            z = Variable(inputs.type(torch.FloatTensor))
            m = torch.zeros_like(inputs)
            m = torch.tensor(m, dtype=torch.float32)
            t = torch.tensor(labels, dtype=torch.long)
        else:
            z, m, t = data['input_ids'], data['attention_mask'], data['labels']

        grad_z_vec = grad_z(z, m, t, model_name, model, gpu=gpu)
...
def calc_influence_function(
    ...
    #     print(list(zip(grad_z_vecs[0], e_s_test))[0])

    for idx1 in range(train_dataset_size):
        #         print('calc_influence_function: ', idx1, train_dataset_size)

        train_influence = []
        for idx2 in range(len(e_s_test)):
            with torch.no_grad():
                tmp_influence = (
                        -sum(
                            [
                                ...
                                for k, j in zip(grad_z_vecs[idx1], e_s_test[idx2])
                                ###################################
            ...
            train_influence.append(tmp_influence)

        influences.append(train_influence)
...
def calc_all_grad_then_test(config, model_name, model, train_loader, test_loader, calculate_if=False, max_length=500, embedding_size=768):
    ...
    def padding(docs_emb):
        # Padding
        b = np.zeros([len(docs_emb), max_length, embedding_size])
        cnt = 0
        # pdb.set_trace()
    ...
    #     print('dadsa', len(train_loader), len(test_loader), config['test_hessian_end_index'])

    #     s_tests, save = calc_s_test(
    #         model,
    #         test_loader,
    #         train_loader,
    #         s_test_outdir,
    ...
    influences, harmful, helpful = calc_influence_function(train_dataset_len, train_z, test_z)
...
# def calc_all_grad(config, model,
#                   train_loader, test_loader,
#                   Mtrain_loader, Mtest_loader,
#                   mode,
#                   traindata, testdata,
...
def calc_all_grad(config, model,
    ...
    for i, test_data in enumerate(test_loader):
        ...
        inputs, values, labels = test_data

        z = Variable(inputs.type(torch.FloatTensor)).cuda()
        m = torch.zeros_like(inputs)
        m = torch.tensor(m, dtype=torch.float32).cuda()
        t = torch.tensor(labels, dtype=torch.long).cuda()
        ...
        #         train_influences = defaultdict(dict)
        ...
        for j, train_data in enumerate(train_loader):
            ...
            inputs, values, labels = train_data

            tz = Variable(inputs.type(torch.FloatTensor)).cuda()
            tm = torch.zeros_like(inputs)
            tm = torch.tensor(tm, dtype=torch.float32).cuda()
            tt = torch.tensor(labels, dtype=torch.long).cuda()
            ...
            influence_results[i][j] = float(score)
