...
def s_test(x_test, m_test, y_test, model, i, samples_loader, gpu=-1, damp=0.01, scale=25.0, loss_func="cross_entropy", clip_value=100):
    ...
    for i, (inputs, values, labels) in enumerate(progress_bar):
        
        
        x_train = Variable(inputs.type(torch.FloatTensor)).cuda()
        m_train = torch.zeros_like(inputs)
        m_train = torch.tensor(m_train, dtype=torch.float32).cuda()
        y_train = torch.tensor(labels, dtype=torch.long).cuda()
...
#            pdb.set_trace()
            out, _ = model(x_train, x_train, x_train, x_train)
...
def grad_z(x, m, y, model_name, model, gpu=-1, graph=None, company_idx=None, loss_func="cross_entropy"):
    ...
    model.train()
    if model_name == 'cosformer':
        prediction, _ = model(x, x, x, x)
        loss = calc_loss(prediction, y, loss_func=loss_func)
    if model_name == 'roberta':
        output = para_model(x, m, labels=y)
        loss = output.loss
    if model_name == 'longformer':
        prediction = para_model(x, m) 
        loss = calc_loss(prediction, y, loss_func=loss_func)
    ...
    if model_name == 'voltage':
        prediction = model(inputs, inputs, inputs, inputs, graph, company_idx)
        loss = calc_loss(prediction, y, loss_func=loss_func)
    # Compute sum of gradients from model parameters to loss
    
#     return grad(loss, model.parameters())
#     params = [ p for p in model.parameters() if p.requires_grad ]
    return list(grad(loss, model.parameters()))
